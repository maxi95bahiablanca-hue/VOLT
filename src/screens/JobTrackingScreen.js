...
const JobTrackingScreen = ({ job: initialJob, session, professional, onComplete, onCancel }) => {
  ...
  const [workerLat, setWorkerLat]   = useState(null);
  const [workerLng, setWorkerLng]   = useState(null);
  ...
  // Detectar cancelación externa (cuando cliente eligió otro profesional)
  useEffect(() => {
    if (job.status === 'cancelled' && isWorker) {
      Alert.alert(
        'Trabajo cancelado',
        'El cliente eligió otro profesional.',
        [{ text: 'Entendido', onPress: onCancel }]
      );
    }
    // Trabajador: el webhook marcó el trabajo como completado → volver al inicio
    if (job.status === 'completed' && isWorker) {
      Alert.alert(
        '¡Pago recibido!',
        'El cliente completó el pago. ¡Buen trabajo!',
        [{ text: 'Volver al inicio', onPress: () => onComplete(job) }]
      );
    }
    // Cliente: el webhook marcó completed pero el browser no redireccionó → navegar igual
    if (job.status === 'completed' && !isWorker) {
      onComplete(job);
    }
  }, [job.status]);
  ...
  useEffect(() => {
    ...
    const channel = jobService.subscribeWorkerLocation(job.professional_id, (locationStr) => {
      // locationStr es "SRID=4326;POINT(lng lat)"
  ...
  const handleWorkerAction = async (action) => {
    ...
    try {
      ...
      if (action === 'arrive') {
        ...
        notifBody  = 'El profesional está en tu domicilio.';
  ...
  const handleClientPay = async () => {
    ...
    try {
      ...
      if (result === 'success') {
        // El webhook de MP notifica al trabajador vía realtime (job.status → completed)
        onComplete(job);
...
L.marker([${job.client_lat || -38.71}, ${job.client_lng || -62.26}],{icon:clientIcon}).addTo(map)
            ...
            <View style={{ flex: 1 }}>
              ...
              {job.work_amount && (
                <Text style={styles.jobAmount}>
                  Total: ${job.work_amount.toLocaleString('es-AR')}
                </Text>
              )}
            </View>
